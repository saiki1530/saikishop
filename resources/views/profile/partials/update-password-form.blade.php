<section>
    ...
    <form method="post" action="{{ route('password.update') }}" class="mt-6 space-y-6">
        ...
        <div>
            ...
            <x-text-input id="current_password" name="current_password" type="password" class="form-input"
        ...
        <div class="flex items-center gap-4">
            <x-primary-button class="custom-button">{{ __('Lưu') }}</x-primary-button>

            @if (session('status') === 'password-updated')
                <p x-data="{ show: true }" x-show="show" x-transition x-init="setTimeout(() => show = false, 2000)"
                    class="text-sm text-gray-600 dark:text-gray-400 text-success">{{ __('Đã lưu.') }}</p>
            @endif
        </div>
    </form>
</section>
